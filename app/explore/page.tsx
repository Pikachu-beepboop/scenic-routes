...
function ExplorePageInner() {
  ...
  async function fetchSavedRoutes() {
    ...
    if (data) {
      setSavedRoutes(data.map((r: any) => r.route_id));
    }
  }
    ...
    if (data) {
      const uniqueCountries = Array.from(
        new Set(
          data
            .map((r: any) => r.country)
            .filter((country: unknown): country is string => typeof country === "string" && country.trim() !== "")
        )
      ).sort();
      ...
      const uniqueDurations = Array.from(
        new Set(
          data
            ...
            .filter((duration: unknown): duration is string => typeof duration === "string" && duration.trim() !== "")
        )
      ).sort();
  ...
  async function fetchRoutes() {
    ...
    if (filters.difficulty.length > 0) {
      query = query.in("terrain", filters.difficulty);
    }
...
:root { --bg:#0c0b09; --bg2:#131109; --bg3:#1a1710; --gold:#C9A86A; --cream:#EDE5D4; --muted:rgba(237,229,212,0.56); --dim:rgba(237,229,212,0.32); --border:rgba(237,229,212,0.12); --serif:'Cormorant Garamond',Georgia,serif; --sans:'Inter',system-ui,sans-serif; }
...
.hero-sub { font-size:20px; font-weight:300; color:rgba(237,229,212,0.58); max-width:780px; line-height:1.6; margin-top:45px; }

/* SEARCH BAR */
.search-bar { display:inline-flex; align-items:stretch; background:rgba(237,229,212,0.06); backdrop-filter:blur(24px); border:1px solid rgba(237,229,212,0.15); border-radius:20px; overflow:visible; width:100%; max-width:1020px; margin-top:36px; }
          ...
          <div className="hero-inner">
            <div className="hero-content">
              ...
              <p className="hero-sub">
                Search through hundreds of handpicked scenic drives — filtered
                by country, duration, terrain and mood.
              </p>
                  ...
                  {isOpen && (
                    <div className="search-dropdown">
                      <div className="search-dropdown-header">
                        <span className="search-dropdown-header-label">
                          Destination
                        </span>
                      </div>
          ...
          <div className="toolbar">
            <p className="results-count">
              {loading ? (
                ...
                <>
                  <strong>{routes.length}</strong> routes found
                </>
              )}
            </p>
                  ...
                  <div className="filter-panel">
                    ...
                    <div className="filter-section">
                      ...
                      <div className="filter-chips">
                        {["Forest", "Deserts", "Coastal", "Mountains"].map(
                          (t) => (
                            <button
                              key={t}
                              className={`filter-chip ${filters.difficulty.includes(t) ? "active" : ""
                                }`}
                              onClick={() => toggleFilter("difficulty", t)}
                            >
                              {t}
                            </button>
                          )
                    ...
                    <div className="filter-section">
                      ...
                      <div className="filter-radio">
                        {[
                          ...
                          { v: "half", l: "Half day (< 4h)" },
                          { v: "full", l: "Full day (4–8h)" },
                          { v: "weekend", l: "Weekend trip" },
                          { v: "multiday", l: "Multi-day journey" },
                        ].map(({ v, l }) => (
                    ...
                    <div className="filter-section">
                      ...
                      <div className="filter-stars">
                        {[1, 2, 3, 4, 5].map((s) => (
                          <button
                            key={s}
                            className={`filter-star ${s <= filters.minRating ? "active" : ""
                              }`}
                            onClick={() =>
                              setFilters((p) => ({
                                ...p,
                                minRating: p.minRating === s ? 0 : s,
                              }))
                            }
                          >
                            ★
            ...
            <div className="active-tags">
              {filters.difficulty.map((t) => (
                <span key={t} className="active-tag">
                  {t}
                  <button onClick={() => toggleFilter("difficulty", t)}>
                    ×
                  </button>
                </span>
              ))}
              ...
              {filters.countries.map((c) => (
                <span key={c} className="active-tag">
                  {c}
                  <button onClick={() => toggleFilter("countries", c)}>
                    ×
                  </button>
                </span>
              ))}
                ...
                <div key={route.id} className="route-card">
                  <div className="route-card-img">
                    <Link href={`/routedetail/${route.id}`}>
                      <img
                        src={route.image_url || "/iceland.jpg"}
                        alt={route.title}
                        onError={(e) => {
                          e.currentTarget.src = "/iceland.jpg";
                        }}
                    ...
                    >
                      <svg
                        ...
                        style={{
                          fill: savedRoutes.includes(route.id)
                            ? "#ef4444"
                            : "transparent",
                          stroke: savedRoutes.includes(route.id)
                            ? "#ef4444"
                            : "rgba(237,229,212,0.8)",
                        }}
                      >
          ...
          <div className="footer-inner">
            <div className="footer-top">
              ...
              {[
                [
                  "Explore",
                  ["All Routes", "Mountain Passes", "Coastal Roads", "Hidden Gems"],
                ],
                [
                  "Discover",
                  ["Weekend Escapes", "Photo Spots", "Driving Roads", "About Us"],
                ],
                [
                  "Support",
                  ["FAQ", "Contact Us", "Privacy Policy", "Terms"],
                ],
              ].map(([h, links]) => (
              ...
              <div>
                ...
                <p className="footer-nl-sub">
                  Get the best scenic routes and travel stories in your inbox.
                </p>
